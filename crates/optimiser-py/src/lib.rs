    ...
    fn with_compaction_format(
        ...
        match RustSmartCrusher::with_compaction_format(cfg, format_name) {
            ...
            None => Err(pyo3::exceptions::PyValueError::new_err(format!(
                ...
                optimiser_core::transforms::smart_crusher::compaction::CompactionStage::SUPPORTED_FORMAT_NAMES.join(", ")
            ))),
        }
    }
...
impl PySearchCompressor {
    ...
    fn compress(
        ...
        let (result, stats) = py.allow_threads(move || {
            let store = optimiser_core::ccr::InMemoryCcrStore::new();
            let (r, s) = self
                .inner
...
///   to `optimiser_core::transforms::live_zone::DEFAULT_MODEL`.
///
/// # Returns
/// `(body, modified)`:
...
fn compress_openai_responses_live_zone(
    ...
    let model_str = if model.is_empty() {
        optimiser_core::transforms::live_zone::DEFAULT_MODEL
    } else {
        model
    };

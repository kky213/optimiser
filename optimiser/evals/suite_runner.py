...
Usage:
    from optimiser.evals.suite_runner import SuiteRunner
...
if TYPE_CHECKING:
    from optimiser.evals.reports.report_card import SuiteResult
...
def _start_proxy(port: int) -> subprocess.Popen | None:
    ...
    try:
        proc = subprocess.Popen(
            [sys.executable, "-m", "optimiser.proxy.server", "--port", str(port)],
...
class SuiteRunner:
    ...
    def _run_lm_eval_benchmark(
        ...
        from optimiser.evals.comprehensive_benchmark import (
            compare_results,
    ...
    def _run_before_after_benchmark(
        ...
        from optimiser.evals.core import EvalMode
        from optimiser.evals.datasets import load_dataset_by_name, load_tool_output_samples
        from optimiser.evals.runners.before_after import BeforeAfterRunner, LLMConfig
    ...
    def run(self) -> SuiteResult:
        ...
        from optimiser.evals.cost_tracker import CostTracker
        from optimiser.evals.reports.report_card import BenchmarkRunResult, SuiteResult
    ...
    def _print_summary(self, result: SuiteResult, tracker: Any) -> None:
        ...
        from optimiser.evals.reports.report_card import generate_markdown

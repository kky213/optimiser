...
if TYPE_CHECKING:
    from optimiser.memory.ports import Embedder, MemoryCache, MemoryStore, TextIndex, VectorIndex


# Extension groups for memory backends registered via setuptools entry points.
_MEMORY_STORE_GROUP = "optimiser.memory_store"
_MEMORY_VECTOR_GROUP = "optimiser.memory_vector"
_MEMORY_TEXT_GROUP = "optimiser.memory_text"
...
def _create_store(config: MemoryConfig) -> MemoryStore:
    ...
    if config.store_backend == StoreBackend.SQLITE:
        from optimiser.memory.adapters.sqlite import SQLiteMemoryStore
...
def _create_embedder(config: MemoryConfig) -> Embedder:
    ...
    with _EMBEDDER_CACHE_LOCK:
        ...
        if config.embedder_backend == EmbedderBackend.LOCAL:
            ...
            from optimiser.memory.adapters.embedders import OllamaEmbedder
...
def _create_vector_index(config: MemoryConfig) -> VectorIndex:
    ...
    if backend == VectorBackend.AUTO:
        from optimiser.memory.adapters import HNSW_AVAILABLE, SQLITE_VEC_AVAILABLE
    ...
    if backend == VectorBackend.SQLITE_VEC:
        from optimiser.memory.adapters import SQLITE_VEC_AVAILABLE
        ...
        from optimiser.memory.adapters.sqlite_vector import SQLiteVectorIndex
    ...
    if backend == VectorBackend.HNSW:
        from optimiser.memory.adapters import HNSW_AVAILABLE
        ...
        from optimiser.memory.adapters.hnsw import HNSWVectorIndex
...
def _create_text_index(config: MemoryConfig) -> TextIndex:
    ...
    if config.text_backend == TextBackend.FTS5:
        from optimiser.memory.adapters.fts5 import FTS5TextIndex
...
def _create_cache(config: MemoryConfig) -> MemoryCache:
    ...
    """
    from optimiser.memory.adapters.cache import LRUMemoryCache

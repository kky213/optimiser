...
Discovers built-in plugins from optimiser.learn.plugins.* and external
...
def _discover() -> dict[str, LearnPlugin]:
    ...
    from optimiser.learn import plugins as plugins_pkg

    for _, mod_name, _ in pkgutil.iter_modules(plugins_pkg.__path__):
        try:
            mod = importlib.import_module(f"optimiser.learn.plugins.{mod_name}")
    ...
    # 2. External: entry_points(group="optimiser.learn_plugin")
    try:
        from importlib.metadata import entry_points

        for ep in entry_points(group="optimiser.learn_plugin"):
            try:
                obj = ep.load()

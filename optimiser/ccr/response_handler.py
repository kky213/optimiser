...
def _optimiser_ccr_learn(hash_key: str, item_count: int) -> None:
    """OPTIMISER: Record that compressed content was retrieved by the model.

    When the model calls headroom_retrieve(), it signals that the compressed
    content was insufficient — the data was important. We log this so future
    sessions can avoid compressing this type of content too aggressively.
    Non-blocking: runs fire-and-forget, never raises.
    """
    try:
        import os
        learn_path = os.path.join(
            os.path.expanduser("~"), ".optimiser", "ccr_retrievals.jsonl"
        )
        ...
        with open(learn_path, "a", encoding="utf-8") as fh:
            fh.write(record + "\n")
        logger.debug(f"[OPTIMISER] CCR retrieval logged: hash={hash_key} items={item_count}")
    except Exception:
        pass
        ...
        try:
            ...
            if ccr_call.query:
                ...
                if entry:
                    ...
                    # OPTIMISER: Learn from CCR retrieval — if the model asked for
                    # full data back, this tool/context needs full data in future.
                    _optimiser_ccr_learn(ccr_call.hash_key, entry.original_item_count)
...
class StreamingCCRHandler:
    ...
    def _parse_sse_stream(self, data: bytes) -> dict[str, Any]:
        ...
        silently corrupted.
        """
        from optimiser.proxy.helpers import parse_sse_events_from_byte_buffer

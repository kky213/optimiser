...
from optimiser.subscription.base import QuotaTracker
from optimiser.subscription.client import SubscriptionClient
from optimiser.subscription.models import (
...
class SubscriptionTracker(QuotaTracker):
    ...
    def _poll_rtk_delta(self) -> int:
        ...
        try:
            ...
            from optimiser.proxy.helpers import _get_rtk_stats
    ...
    def _compute_used_since_reset(self, window: RateLimitWindow | None) -> int | None:
        ...
        if now < window.resets_at:
            return None
        try:
            from optimiser.subscription import session_tracking
    ...
    async def _maybe_poll(self) -> None:
        ...
        if not is_active:
            ...
            from optimiser.subscription.client import read_cached_oauth_token
        ...
        try:
            from optimiser.observability.metrics import get_otel_metrics

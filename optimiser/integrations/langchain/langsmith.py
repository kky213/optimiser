...
Example:
    ...
    from optimiser.integrations import (
        HeadroomChatModel,
...
class HeadroomLangSmithCallbackHandler(BaseCallbackHandler):
    ...
    def _attach_metrics_to_run(self, run_id: str, metrics: PendingMetrics) -> None:
        ...
        """
        metadata = {
            "optimiser.tokens_before": metrics.tokens_before,
            "optimiser.tokens_after": metrics.tokens_after,
            "optimiser.tokens_saved": metrics.tokens_saved,
            "optimiser.savings_percent": round(metrics.savings_percent, 2),
            "optimiser.transforms_applied": metrics.transforms_applied,
            "optimiser.optimization_timestamp": metrics.timestamp.isoformat(),
        }
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        total_saved = sum(m.get("optimiser.tokens_saved", 0) for m in self._run_metrics.values())
        savings_percents = [
            m.get("optimiser.savings_percent", 0) for m in self._run_metrics.values()
        ]

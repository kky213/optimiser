...
from optimiser.agent_savings import proxy_pipeline_kwargs
from optimiser.cache.compression_feedback import get_compression_feedback
from optimiser.cache.compression_store import format_retrieval_miss_detail, get_compression_store
from optimiser.ccr import (
    CCR_TOOL_NAME,
    # Batch processing
    CCRResponseHandler,
    CCRToolInjector,
...
from optimiser.dashboard import get_dashboard_html
from optimiser.observability import (
...
from optimiser.pipeline import PipelineExtensionManager, PipelineStage
from optimiser.providers.proxy_routes import register_provider_routes
from optimiser.providers.registry import (
...
from optimiser.proxy import runtime_env
from optimiser.proxy.auth_mode import should_stamp_codex_client

# =============================================================================
# Extracted modules (re-exported for backward compatibility)
# =============================================================================
from optimiser.proxy.cost import (
...
from optimiser.proxy.helpers import (
...
from optimiser.proxy.memory_handler import MemoryConfig, MemoryHandler

# Data models (extracted to headroom/proxy/models.py for maintainability)
from optimiser.proxy.models import CacheEntry, ProxyConfig, RateLimitState, RequestLog  # noqa: F401
from optimiser.proxy.modes import (
    PROXY_MODE_CACHE,
    PROXY_MODE_TOKEN,
    is_token_mode,
    normalize_proxy_mode,
)
from optimiser.proxy.probe_recorder import probe_recorder_from_env
from optimiser.proxy.project_context import (
    classify_project,
    set_current_project,
    strip_project_path_prefix,
)
from optimiser.proxy.prometheus_metrics import PrometheusMetrics  # noqa: F401
from optimiser.proxy.rate_limiter import TokenBucketRateLimiter  # noqa: F401
from optimiser.proxy.request_logger import RequestLogger  # noqa: F401
from optimiser.proxy.semantic_cache import SemanticCache  # noqa: F401
from optimiser.proxy.ssl_context import find_ca_bundle
from optimiser.proxy.warmup import WarmupRegistry
from optimiser.proxy.ws_session_registry import WebSocketSessionRegistry
from optimiser.subscription.base import get_quota_registry, reset_quota_registry
from optimiser.subscription.codex_rate_limits import get_codex_rate_limit_state
from optimiser.subscription.copilot_quota import get_copilot_quota_tracker
from optimiser.subscription.tracker import (
    configure_subscription_tracker,
    get_subscription_tracker,
)
from optimiser.telemetry import get_telemetry_collector
from optimiser.telemetry.beacon import is_telemetry_enabled
from optimiser.telemetry.toin import get_toin
from optimiser.transforms import (
    CacheAligner,
    CodeAwareCompressor,
...
logger = logging.getLogger("optimiser.proxy")
...
# Default behavior: hard-fail at startup if `optimiser._core` is unimportable
...
def _check_rust_core() -> tuple[str, str | None]:
    """Verify the Rust extension `optimiser._core` is loadable at startup.

    Returns a `(status, error)` tuple:
      - ``("loaded", None)``     — `optimiser._core.hello()` returned the
        expected sentinel.
    ...
    try:
        from optimiser._core import hello as _rust_hello
        ...
        msg = (
            f"FATAL: Rust extension `optimiser._core` not loadable.\n"
            f"    error: {reason}\n"
    ...
    if marker != "optimiser-core":
        ...
        if not require:
            ...
            return ("disabled", reason)
        msg = (
            f"FATAL: Rust extension `optimiser._core` is loaded but the "
...
from optimiser.proxy.handlers import (  # noqa: E402
...
class HeadroomProxy(
    ...
    def __init__(self, config: ProxyConfig):
        ...
        from optimiser.cache.prefix_tracker import PrefixFreezeConfig, SessionTrackerStore
        ...
        if config.memory_enabled:
            ...
            from optimiser.proxy.memory_handler import MemoryMode
            ...
            from optimiser.memory.storage_router import MemoryStorageMode
        ...
        if config.license_key:
            from optimiser.telemetry.reporter import UsageReporter
        ...
        if config.traffic_learning_enabled:
            from optimiser.memory.traffic_learner import TrafficLearner
        ...
        if config.code_graph_watcher:
            from optimiser.graph.watcher import CodeGraphWatcher
    ...
    def _get_compression_cache(self, session_id: str) -> CompressionCache:
        ...
        with self._compression_caches_lock:
            if session_id not in self._compression_caches:
                from optimiser.cache.compression_cache import CompressionCache
        ...
        if config.code_aware_enabled:
            if is_tree_sitter_available():
                ...
                logger.warning(
                    ...
                    "Install with: pip install optimiser-ai[code]"
                )
                return "unavailable"
        else:
            if is_tree_sitter_available():
    ...
    async def startup(self):
        ...
        if self._kompress_status == "enabled":
            ...
            logger.info("Kompress: not installed (pip install optimiser-ai[ml] for ML compression)")
        ...
        if self._code_aware_status == "enabled":
            ...
            logger.info("Code-Aware: not installed (pip install optimiser-ai[code])")
    ...
    async def shutdown(self):
        ...
        with contextlib.suppress(Exception):
            from optimiser.models.ml_models import MLModelRegistry
    ...
    async def _record_request_outcome(self, outcome: RequestOutcome) -> None:
        ...
        """
        from optimiser.proxy.outcome import emit_request_outcome
    ...
    def _extract_tags(self, headers: dict) -> dict[str, str]:
        ...
        """
        from optimiser.proxy.helpers import extract_tags
...
def create_app(config: ProxyConfig | None = None) -> FastAPI:
    ...
    from optimiser.proxy.cc_switch_reconciler import (
        CCSwitchReconciler,
        reconciler_enabled,
    )
    ...
    if reconciler_enabled():
        ...
        def _set_anthropic_upstream(url: str) -> None:
            from optimiser.providers.registry import _normalize_api_url
    ...
    from optimiser.telemetry.beacon import TelemetryBeacon
    ...
    from optimiser import paths as _hr_paths
    ...
    async def lifespan(app: FastAPI):  # type: ignore[no-untyped-def]
        ...
        configure_otel_metrics(OTelMetricsConfig.from_env(default_service_name="optimiser-proxy"))
        configure_langfuse_tracing(
            LangfuseTracingConfig.from_env(default_service_name="optimiser-proxy")
        )
    ...
    async def _record_headroom_stack(request, call_next):
        ...
        try:
            from optimiser.proxy.helpers import redact_for_wire_debug
    ...
    from optimiser.proxy.extensions import install_all as _install_extensions
    ...
    from optimiser.proxy.debug_introspection import (
        collect_tasks as _collect_tasks,
    )
    from optimiser.proxy.loopback_guard import require_loopback as _require_loopback
    ...
    async def _build_stats_payload() -> dict[str, Any]:
        ...
        async with _throughput_cache_lock:
            ...
            if _throughput_cache["expires_at"] < now or _throughput_cache["value"] is None:
                ...
                def _compute_throughput():
                    from optimiser.perf.analyzer import build_perf_summary, parse_log_files
        ...
        try:
            from optimiser.proxy.output_savings import get_recorder
...
def _get_code_aware_banner_status(config: ProxyConfig) -> str:
    ...
    if config.code_aware_enabled:
        if is_tree_sitter_available():
            ...
            return "NOT INSTALLED (pip install optimiser-ai[code])"
    else:
        if is_tree_sitter_available():
...
""")
    ...
    if workers > 1:
        ...
        app_target = "optimiser.proxy.server:create_app_from_env"
...
def _parse_tool_profiles(cli_profiles: list[str]) -> dict[str, Any]:
    ...
    """
    from optimiser.config import PROFILE_PRESETS, CompressionProfile
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--code-aware",
        action="store_true",
        help="Enable AST-based code compression (requires: pip install optimiser-ai[code])",
    )
    parser.add_argument(
        "--no-code-aware",
        action="store_true",

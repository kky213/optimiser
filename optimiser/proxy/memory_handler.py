...
from optimiser.memory import qdrant_env
...
if TYPE_CHECKING:
    from optimiser.memory.backends.local import LocalBackend
...
class MemoryHandler:
    ...
    async def _init_backend_locked(self) -> None:
        ...
        if self.config.backend == "local":
            ...
            try:
                from optimiser.memory.backends.direct_mem0 import (
                    DirectMem0Adapter,
                    Mem0Config,
                )
    ...
    async def _init_and_import_bridge(self) -> None:
        ...
        if self._bridge is not None:
            return
        try:
            from optimiser.memory.bridge import MemoryBridge
    ...
    def _get_memory_tools(self) -> list[dict[str, Any]]:
        ...
        if self._memory_tools is None:
            from optimiser.memory.tools import get_memory_tools_optimized
    ...
    async def search_and_format_context(
        ...
        *,
        ...
        """
        from optimiser.proxy.memory_injection import MemoryInjectionBudget
        ...
        try:
            ...
            if ranker is not None:
                from optimiser.proxy.memory_ranker import MemoryCandidate
...
your responses, not to drive new actions."""
    ...
    def _append_to_latest_user_tail(
        ...
        *,
        ...
        if provider == "openai":
            from optimiser.proxy.helpers import append_text_to_latest_user_chat_message

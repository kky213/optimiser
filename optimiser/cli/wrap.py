...
from optimiser._version import __version__ as _HEADROOM_VERSION
from optimiser.agent_savings import (
    apply_agent_savings_env_defaults,
)
from optimiser.copilot_auth import (
    has_oauth_auth,
    resolve_client_bearer_token,
    resolve_copilot_api_url,
    resolve_subscription_bearer_token_details,
)
from optimiser.providers.aider import build_launch_env as _build_aider_launch_env
from optimiser.providers.claude import (
    TOOL_SEARCH_DEFAULT,
    TOOL_SEARCH_ENV,
)
from optimiser.providers.claude import (
...
from optimiser.providers.codex import build_launch_env as _build_codex_launch_env
from optimiser.providers.codex.install import codex_uses_chatgpt_auth
from optimiser.providers.codex.threads import retag_to_headroom, retag_to_native
from optimiser.providers.copilot import (
...
from optimiser.providers.cursor import render_setup_lines as _render_cursor_setup_lines
from optimiser.providers.mistral_vibe import build_launch_env as _build_mistral_vibe_launch_env
from optimiser.providers.openclaw import (
...
from optimiser.proxy.project_context import with_project_prefix as _with_project_prefix
...
def _print_telemetry_notice() -> None:
    ...
    """
    from optimiser.telemetry.beacon import format_telemetry_notice
...
def _get_log_path() -> Path:
    ...
    from optimiser import paths as _paths
...
def _start_proxy(
    ...
    *,
    ...
    """
    cmd = [sys.executable, "-m", "optimiser.cli", "proxy", "--port", str(port)]
...
def _setup_rtk(verbose: bool = False) -> Path | None:
    ...
    from optimiser.rtk import get_rtk_path
    from optimiser.rtk.installer import ensure_rtk, register_claude_hooks
...
def _setup_lean_ctx_agent(agent: str, verbose: bool = False) -> Path | None:
    ...
    from optimiser.lean_ctx import get_lean_ctx_path
    from optimiser.lean_ctx.installer import ensure_lean_ctx
...
def _setup_headroom_mcp(
    ...
    """
    from optimiser.mcp_registry import build_headroom_spec, format_result
...
def _setup_serena_mcp(
    ...
    """
    from optimiser.mcp_registry import build_serena_spec, format_result
    from optimiser.mcp_registry.base import RegisterStatus
    from optimiser.mcp_registry.ledger import headroom_installed_matching, record_install
...
def _remove_headroom_installed_serena_mcp(registrar: Any) -> str:
    ...
    from optimiser.mcp_registry.ledger import clear_install, headroom_installed_matching
...
def _setup_code_graph(verbose: bool = False) -> bool:
    ...
    """
    from optimiser.graph.installer import ensure_cbm, get_cbm_path
...
def _ensure_rtk_binary(verbose: bool = False) -> Path | None:
    ...
    from optimiser.rtk import get_rtk_path
    from optimiser.rtk.installer import ensure_rtk
...
def _inject_memory_mcp_config(db_path: str, user_id: str) -> None:
    ...
    mcp_section = (
        ...
        f'args = ["-m", "optimiser.memory.mcp_server", "--db", "{db_path_toml}", "--user", "{user_id}"]\n'
...
def _find_persistent_manifest(port: int) -> Any:
    ...
    from optimiser.install.state import list_manifests
...
def _recover_persistent_proxy(port: int) -> bool:
    ...
    from optimiser.install.health import probe_ready
    from optimiser.install.models import InstallPreset, SupervisorKind
    from optimiser.install.runtime import start_detached_agent, start_persistent_docker, wait_ready
    from optimiser.install.supervisors import start_supervisor
...
def _restart_persistent_proxy(manifest: Any, port: int) -> bool:
    ...
    from optimiser.install.models import InstallPreset, SupervisorKind
    from optimiser.install.runtime import (
...
def _push_runtime_env(port: int, no_proxy: bool) -> None:
    ...
    if no_proxy:
        return
    from optimiser.proxy import runtime_env as _rt
...
def claude(
    ...
    try:
        ...
        if not no_mcp:
            from optimiser.mcp_registry import ClaudeRegistrar
...
def codex(
    ...
    if not no_mcp:
        from optimiser.mcp_registry import CodexRegistrar
    ...
    if memory:
        ...
        try:
            ...
            from optimiser.memory.backends.local import LocalBackend, LocalBackendConfig
            from optimiser.memory.sync import sync_import
            from optimiser.memory.sync_adapters.claude_code import (
                ClaudeCodeAdapter,

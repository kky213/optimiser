...
from optimiser.providers.registry import resolve_api_overrides, resolve_api_targets
from optimiser.proxy.modes import PROXY_MODE_TOKEN, normalize_proxy_mode
...
def proxy(
    ...
    try:
        from optimiser.proxy.server import (
            ProxyConfig,
    ...
    if intercept_tool_results:
        from optimiser.binaries import ensure_tools
    ...
    # OPTIMISER: memory on by default; disable with OPTIMISER_MEMORY=0
    _optimiser_memory_default = os.environ.get("OPTIMISER_MEMORY", "1").strip().lower() not in ("0", "false", "no")
    ...
    config = ProxyConfig(
        ...
        # 2. Otherwise read OPTIMISER_CODE_AWARE_ENABLED (falsy = off).
        # 3. Otherwise default ON in Optimiser — AST compression active by default.
        code_aware_enabled=(
            bool(code_aware_flag)
            if code_aware_flag is not None
            else os.environ.get("OPTIMISER_CODE_AWARE_ENABLED", "1").strip().lower()
        ...
        # Memory ON by default in Optimiser (project-scoped SQLite).
        # Disable with OPTIMISER_MEMORY=0 or --stateless.
        memory_enabled=False if is_stateless else (memory or _optimiser_memory_default or (learn and not no_learn)),
...
"""
        ...
        # LiteLLM backend
        from optimiser.backends.litellm import get_provider_config
...
"""
    ...
    try:
        from optimiser.proxy.extensions import discover as _discover_extensions
    ...
    from optimiser.proxy.server import _get_code_aware_banner_status
...
╔═══════════════════════════════════════════════════════════════════════╗
║                        OPTIMISER  v1.0                                 ║
║         AI Token Saver — Compress LLM context by 55-80%               ║
╚═══════════════════════════════════════════════════════════════════════╝

Starting Optimiser proxy...
...
""")
    ...
    try:
        from optimiser.update_check import format_update_notice
    ...
    if embedding_server:
        ...
        from optimiser.memory.adapters.watchdog import EmbeddingServerWatchdog

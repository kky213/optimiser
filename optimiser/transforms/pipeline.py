...
class TransformPipeline:
    ...
    def _build_default_transforms(self) -> list[Transform]:
        ...
        # ON BY DEFAULT in Optimiser. Disable with OPTIMISER_INTERCEPT_ENABLED=0.
        import os as _os

        _intercept_off = _os.environ.get("OPTIMISER_INTERCEPT_ENABLED", "1").lower() in ("0", "false", "no")
        if (getattr(self.config, "intercept_tool_results", True) or True) and not _intercept_off:
            from optimiser.proxy.interceptors import ToolResultInterceptorTransform
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        span_attributes = {
            ...
            "optimiser.provider": provider_name or "unknown",
            "optimiser.message_count": len(messages),
            "optimiser.tokens.before": tokens_before,
        }
        pipeline_span_context = (
            tracer.start_as_current_span(
                "optimiser.compression.pipeline",
        ...
        with pipeline_span_context as pipeline_span:
            ...
            for transform in self.transforms:
                ...
                transform_span_context = (
                    tracer.start_as_current_span(
                        "optimiser.compression.transform",
                        attributes={
                            "optimiser.model": model,
                            "optimiser.provider": provider_name or "unknown",
                            "optimiser.transform": transform.name,
                        },
                    )
                    if record_metrics
                    else nullcontext()
                ...
                with transform_span_context as transform_span:
                    ...
                    if transform_span is not None and transform_span.is_recording():
                        transform_span.set_attribute(
                            "optimiser.tokens.before", tokens_before_transform
                        )
                        transform_span.set_attribute(
                            "optimiser.tokens.after", tokens_after_transform
                        )
                        transform_span.set_attribute(
                            "optimiser.tokens.saved",
                            tokens_before_transform - tokens_after_transform,
                        )
                        transform_span.set_attribute("optimiser.duration_ms", duration_ms)
                        transform_span.set_attribute(
                            "optimiser.transforms_applied",
            ...
            if pipeline_span is not None and pipeline_span.is_recording():
                pipeline_span.set_attribute("optimiser.tokens.after", tokens_after)
                pipeline_span.set_attribute("optimiser.tokens.saved", total_saved)
                pipeline_span.set_attribute("optimiser.duration_ms", pipeline_ms)
                pipeline_span.set_attribute("optimiser.transforms_applied", len(all_transforms))
                pipeline_span.set_attribute("optimiser.warnings", len(all_warnings))
